...
def custom_error_exception(error=None, exception=None):
    """Define custom exceptions for GBase server errors

    This function defines custom exceptions for GBase server errors and
    returns the current set customizations.

    If error is a GBase Server error number, then you have to pass also the
    exception class.
    ...
    Examples:
        ...
        from GBaseConnector import errorcode

        # Server error 1028 should raise a DatabaseError
        GBaseConnector.custom_error_exception(
            1028, GBaseConnector.DatabaseError)

        # Or using a dictionary:
        GBaseConnector.custom_error_exception({
            1028: GBaseConnector.DatabaseError,
            1029: GBaseConnector.OperationalError,
            })
...
def get_gbase_exception(errno, msg=None, sqlstate=None):
    """Get the exception matching the GBase error
    ...
    GBaseConnector.custom_error_exception() function.
...
def get_exception(packet):
    """Returns an exception object based on the GBase error

    Returns an exception object based on the GBase error in the given
    packet.
    ...
    try:
        ...
        return get_gbase_exception(errno, errmsg, sqlstate)

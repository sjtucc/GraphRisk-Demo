...
from .conversion import GBaseConverterBase
from .GBaseConstants import (ClientFlag, CharacterSet, CONN_ATTRS_DN,
                             DEFAULT_CONFIGURATION, OPENSSL_CS_NAMES,
                             TLS_CIPHER_SUITES, TLS_VERSIONS)
from .optionfiles import GBaseOptionsParser
...
@make_abc(ABCMeta)
class GBaseConnectionAbstract(object):

    """Abstract class for classes connecting to a GBase server"""
    ...
    def _read_option_files(self, config):
        ...
        if 'option_files' in config:
            ...
            option_parser = GBaseOptionsParser(list(config['option_files']),
                                               keep_dashes=False)
    ...
    @property
    def user(self):
        """User used while connecting to GBase"""
        return self._user

    @property
    def server_host(self):
        """GBase server IP address or name"""
        return self._host

    @property
    def server_port(self):
        "GBase server TCP/IP port"
        return self._port

    @property
    def unix_socket(self):
        "GBase Unix socket file location"
    ...
    def config(self, **kwargs):
        """Configure the GBase Connection

        This method allows you to configure the GBaseConnection instance.

        Raises on errors.
        """
        config = kwargs.copy()
        if 'dsn' in config:
            raise GBaseError.NotSupportedError("Data source name is not supported")

        # Read option files
        self._read_option_files(config)

        # Configure how we handle GBase warnings
        try:
        ...
        try:
            ...
            raise AttributeError("Converter class should be a subclass "
                                 "of conversion.GBaseConverterBase.")
        ...
        if self._conn_attrs is None:
            ...
            raise GBaseError.InterfaceError('conn_attrs must be of type dict.')
        else:
    ...
    def _check_server_version(self, server_version):
        """Check the GBase version

        This method will check the GBase version and raise an InterfaceError
        ...
        if not match:
            raise GBaseError.InterfaceError("Failed parsing GBase version")

        version = tuple([int(v) for v in match.groups()[0:3]])
        if version < (4, 1):
            raise GBaseError.InterfaceError(
                "GBase Version '{0}' is not supported.".format(server_version))

        return version

    def get_server_version(self):
        """Get the GBase version

        This method returns the GBase server version as a tuple. If not
    ...
    def get_server_info(self):
        """Get the original GBase version information

        This method returns the original GBase server as text. If not
        previously connected, it will return None.
    ...
    def sql_mode(self, value):
        ...
        It is good practice to use the constants class SQLMode:
          from GBaseConnector.constants import SQLMode
    ...
    def set_login(self, username=None, password=None):
        """Set login information for GBase
    ...
    def raise_on_warnings(self, value):
        ...
        The toggle-argument must be a boolean. When True, cursors for this
        connection will raise an error when GBase reports warnings.
    ...
    def connect(self, **kwargs):
        """Connect to the GBase server

        This method sets up the connection to the GBase server. If no
    ...
    def start_transaction(self, consistent_snapshot=False,
        ...
        START TRANSACTION statement to the GBase server. You can optionally
        ...
        if self.in_transaction:
            raise GBaseError.ProgrammingError("Transaction already in progress")
        ...
        if readonly is not None:
            if self._server_version < (5, 6, 5):
                raise ValueError(
                    "GBase server version {0} does not support "
    ...
    def reset_session(self, user_variables=None, session_variables=None):
        ...
        except (GBaseError.NotSupportedError, NotImplementedError):
            if self._compress:
                raise GBaseError.NotSupportedError(
                    "Reset session is not supported with compression for "
                    "GBase server version 5.7.2 or earlier.")
            else:
    ...
    def set_converter_class(self, convclass):
        ...
        methods and members of conversion.GBaseConverter.
        """
        if convclass and issubclass(convclass, GBaseConverterBase):
    ...
    def cmd_process_info(self):
        """Get the process list of the GBase Server
...
@make_abc(ABCMeta)
class GBaseCursorAbstract(object):

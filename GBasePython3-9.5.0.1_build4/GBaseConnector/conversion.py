"""Converting GBase and Python types
"""
...
from .GBaseConstants import FieldType, FieldFlag, CharacterSet
...
class GBaseConverterBase(object):
    """Base class for conversion classes

    All class dealing with converting to and from GBase data types must
    ...
    def to_gbase(self, value):
        """Convert Python data type to GBase"""
        type_name = value.__class__.__name__.lower()
        try:
            return getattr(self, "_{0}_to_gbase".format(type_name))(value)
        except AttributeError:
            return value

    def to_python(self, vtype, value):
        """Convert GBase data type to Python"""
    ...
    def escape(self, value):
        """Escape buffer for sending to GBase"""
        return value

    def quote(self, buf):
        """Quote buffer for sending to GBase"""
        return str(buf)


class GBaseConverter(GBaseConverterBase):
    """Default conversion class for GBase Connector/Python.

     o escape method: for escaping values send to GBase
     o quoting method: for quoting values send to GBase in statements
     o conversion mapping: maps Python and GBase data types to
    ...
    cnx.connect(converter_class=CustomGBaseConverterClass).

    """

    def __init__(self, charset=None, use_unicode=True):
        GBaseConverterBase.__init__(self, charset, use_unicode)
        self._cache_field_types = {}
    ...
    def escape(self, value):
        ...
        receives them.
        As found in GBase source mysys/charset.c
    ...
    def _str_to_gbase(self, value):
        ...
        if PY2:
            return str(value)
        return self._unicode_to_gbase(value)
    ...
    def _bytes_to_gbase(self, value):
        """Convert value to bytes"""
        return value

    def _bytearray_to_gbase(self, value):
        """Convert value to bytes"""
        return bytes(value)
    ...
    def row_to_python(self, row, fields):
        ...
        by a GBase server. Each value of the row is converted to the
    ...
    def _DATE_to_python(self, value, dsc=None):  # pylint: disable=C0103
        """Converts TIME column GBase to a python datetime.datetime type.
    ...
    def _TIME_to_python(self, value, dsc=None):  # pylint: disable=C0103
        ...
        Converts the TIME column GBase type passed as bytes to a python
    ...
    def _DATETIME_to_python(self, value, dsc=None):  # pylint: disable=C0103
        ...
        Converts the DATETIME column GBase type passed as bytes to a python
        datetime.datetime type.
        ...
        try:
            ...
            if len(dtval) < 6:
                ...
                # Note that by default GBase accepts invalid timestamps

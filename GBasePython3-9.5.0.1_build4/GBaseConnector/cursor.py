...
from .abstracts import GBaseCursorAbstract, NAMED_TUPLE_CACHE
from .catch23 import PY2
from .GBaseConstants import ServerFlag
...
class CursorBase(GBaseCursorAbstract):
    """
    Base for defining GBaseCursor. This class is a skeleton and defines
    methods and members as required for the Python Database API
    Specification v2.0.

    It's better to inherite from GBaseCursor.
    """
    ...
    def callproc(self, procname, args=()):
        ...
        Coding Example:
          1) Definining the Stored Routine in GBase:
    ...
    def execute(self, operation, params=(), multi_stmt=False):
        ...
        The multi_stmt argument should be set to True when executing multiple
        ...
        Returns an iterator when multi_stmt is True, otherwise None.
        """
        pass
...
class GBaseCursor(CursorBase):
    """Default cursor for interacting with GBase

    This cursor will execute statements and handle the result. It will
    not automatically fetch all rows.

    GBaseCursor should be inherited whenever other functionallity is
    ...
    def _process_params_dict(self, params):
        ...
        try:
            to_gbase = self._connection.converter.to_gbase
    ...
    def _handle_noresultset(self, res):
        ...
        if self._connection.raise_on_warnings is True and self._warnings:
            raise GBaseError.get_gbase_exception(
    ...
    def _handle_result(self, result):
        ...
        if not isinstance(result, dict):
            raise GBaseError.InterfaceError('Result was not a dict()')
        ...
        if 'columns' in result:
            ...
            raise GBaseError.InterfaceError('Invalid result')

    def _execute_iter(self, query_iter):
        """Generator returns GBaseCursor objects for multiple statements
        ...
        given query_iter (result of GBaseConnection.cmd_query_iter()) and
    ...
    def execute(self, operation, params=None, multi_stmt=False):
        ...
        if not self._connection:
            raise GBaseError.ProgrammingError("Cursor is not connected")
        ...
        try:
            ...
            raise GBaseError.ProgrammingError(str(err))
        ...
        try:
            ...
            if self._connection._have_next_result:  # pylint: disable=W0212
                raise GBaseError.InterfaceError(
                    "Use multi_stmt=True when executing multiple statements")
            raise
        return None

    def _batch_insert(self, operation, seq_params):
        """Implements multi_stmt row insert"""
        ...
        if not matches:
            raise GBaseError.InterfaceError(
                "Failed rewriting statement for multi_stmt-row INSERT. "
                "Check SQL syntax."
            )
    ...
    def stored_results(self):
        ...
        callproc() is called. The iterator will provide GBaseCursorBuffered
        instances.
    ...
    def callproc(self, procname, args=()):
        ...
        Coding Example:
          1) Defining the Stored Routine in GBase:
          CREATE PROCEDURE multiply(IN pFac1 INT, IN pFac2 INT, OUT pProd INT)
          BEGIN
            SET pProd := pFac1 * pFac2;
          END
        ...
        the GBase CAST function to convert the values in the corresponding
        GBase type (See CAST in GBase Reference for more information)
        ...
        try:
            ...
            for result in self._connection.cmd_query_iter(call):
                ...
                if self._raw:
                    tmp = GBaseCursorBufferedRaw(self._connection._get_self())
                else:
                    tmp = GBaseCursorBuffered(self._connection._get_self())
    ...
    def fetchall(self):
        if not self._have_unread_result():
            raise GBaseError.InterfaceError("No result set to fetch from.")
...
class GBaseCursorBuffered(GBaseCursor):
    """Cursor which fetches rows within execute()"""

    def __init__(self, connection=None):
        GBaseCursor.__init__(self, connection)
        self._rows = None
        self._next_row = 0
...
class GBaseCursorRaw(GBaseCursor):
    """
    Skips conversion from GBase datatypes to Python types when fetching rows.
    """
...
class GBaseCursorBufferedRaw(GBaseCursorBuffered):
    """
    Cursor which skips conversion from GBase datatypes to Python types when
...
class GBaseCursorPrepared(GBaseCursor):
    """Cursor using GBase Prepared Statements
    """
    def __init__(self, connection=None):
        super(GBaseCursorPrepared, self).__init__(connection)
        self._rows = None
        self._next_row = 0
        self._prepared = None
        self._binary = True
    ...
    def _handle_noresultset(self, res):
        ...
        super(GBaseCursorPrepared, self)._handle_noresultset(res)
    ...
    def _handle_eof(self, eof):
        ...
        super(GBaseCursorPrepared, self)._handle_eof(eof)

    def callproc(self, procname, args=()):
        """Calls a stored procedue

        Not supported with GBaseCursorPrepared.
        """
    ...
    def close(self):
        ...
        """
        self.reset()
        super(GBaseCursorPrepared, self).close()

    def _row_to_python(self, rowdata, desc=None):
        """Convert row data from GBase to Python types
    ...
    def execute(self, operation, params=(), multi_stmt=False):  # multi_stmt is unused
        """Prepare and execute a GBase Prepared Statement
        ...
        if operation is not self._executed:
            ...
            # need to convert %s to ? before sending it to GBase
            if b'%s' in operation:
    ...
    def executemany(self, operation, seq_params):
        """Prepare and execute a GBase Prepared Statement many times
...
class GBaseCursorDict(GBaseCursor):
    """
    ...
    """
    def _row_to_python(self, rowdata, desc=None):
        """Convert a GBase text result row to Python types
    ...
    def fetchall(self):
        ...
        """
        if not self._have_unread_result():
            raise GBaseError.InterfaceError(ERR_NO_RESULT_TO_FETCH)
        (rows, eof) = self._connection.get_rows()
        if self._nextrow[0]:
            rows.insert(0, self._nextrow[0])
        res = []
...
class GBaseCursorNamedTuple(GBaseCursor):
    """
...
class GBaseCursorBufferedDict(GBaseCursorDict, GBaseCursorBuffered):
    """
...
class GBaseCursorBufferedNamedTuple(GBaseCursorNamedTuple, GBaseCursorBuffered):
    """

"""Various GBase constants and character sets
"""

import ssl

from .GBaseError import ProgrammingError
from .charsets import GBASE_CHARACTER_SETS

MAX_PACKET_LENGTH = 16777215
NET_BUFFER_LENGTH = 8192
MAX_GBASE_TABLE_COLUMNS = 4096
...
class FieldFlag(_Flags):
    ...
    Field flags as found in GBase sources gbase-src/include/gbase_com.h
    """
    _prefix = ''
    NOT_NULL = 1 << 0
    PRI_KEY = 1 << 1
...
class ClientFlag(_Flags):
    ...
    Client options as found in the GBase sources gbase-src/include/gbase_com.h
    """
    LONG_PASSWD = 1 << 0
    FOUND_ROWS = 1 << 1
    LONG_FLAG = 1 << 2
...
class ServerFlag(_Flags):
    ...
    Server flags as found in the GBase sources gbase-src/include/gbase_com.h
    """
    _prefix = 'SERVER_'
    STATUS_IN_TRANS = 1 << 0
...
class CharacterSet(_Constants):
    """GBase supported character sets and collations

    List of character sets with their collations supported by GBase. This
...
class SQLMode(_Constants):
    ...
    are used when setting the SQL_MODE system variable using the GBase
    SET command.

    See http://dev.gbase.com/doc/refman/5.6/en/server-sql-mode.html
    """
    _prefix = 'MODE_'

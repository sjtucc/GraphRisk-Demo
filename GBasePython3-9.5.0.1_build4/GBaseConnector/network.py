"""Module implementing low-level socket communication with GBase servers.
"""
...
from . import GBaseConstants, GBaseError
from .GBaseError import InterfaceError
...
def _prepare_packets(buf, pktnr):
    """Prepare a packet for sending to the GBase server"""
    pkts = []
    pllen = len(buf)
    maxpktlen = GBaseConstants.MAX_PACKET_LENGTH
...
class BaseGBaseSocket(object):
    """Base class for GBase socket communication
    ...
    at least the open_connection()-method. Examples of subclasses are
      GBaseConnector.network.GBaseTCPSocket
      GBaseConnector.network.GBaseUnixSocket
    """
    ...
    def send_plain(self, buf, packet_number=None,
        ...
        """Send packets to the GBase server"""
        ...
        for packet in packets:
            try:
                ...
                raise GBaseError.OperationalError(errno=2006)

    send = send_plain

    def send_compressed(self, buf, packet_number=None,
                        compressed_packet_number=None):
        """Send compressed packets to the GBase server"""
        if packet_number is None:
            self.next_packet_number  # pylint: disable=W0104
        else:
            self._packet_number = packet_number
    ...
    def recv_plain(self):
        """Receive packets from the GBase server"""
        try:
            # Read the header of the GBase packet, 4 bytes
            packet = bytearray(b'')
            packet_len = 0
            while packet_len < 4:
    ...
    def recv_compressed(self):
        """Receive compressed packets from the GBase server"""
        try:
        ...
        # Compressed packet can contain more than 1 GBase packets
...
class GBaseUnixSocket(BaseGBaseSocket):
    """GBase socket class using UNIX sockets

    Opens a connection through the UNIX socket of the GBase Server.
    """

    def __init__(self, unix_socket='/tmp/gbase.sock'):
        super(GBaseUnixSocket, self).__init__()
...
class GBaseTCPSocket(BaseGBaseSocket):
    """GBase socket class using TCP/IP

    Opens a TCP/IP connection to the GBase Server.
    """

    def __init__(self, host='127.0.0.1', port=3306, force_ipv6=False, connection_timeout=None):
        super(GBaseTCPSocket, self).__init__()
    ...
    def open_connection(self):
        """Open the TCP/IP connection to the GBase server
        """
        ...
        try:
            ...
            raise GBaseError.OperationalError(str(err))

"""Implements the GBase Client/Server protocol
"""
...
from . import GBaseError, utils
from .authentication import get_auth_plugin
from .catch23 import PY2, struct_unpack
from .GBaseError import DatabaseError, get_exception

PROTOCOL_VERSION = 10


class GBaseProtocol(object):
    """Implements GBase client/server protocol

    Create and parses GBase packets.
    """
    ...
    def _auth_response(self, client_flags, username, password, database,
        ...
        except (TypeError, GBaseError.InterfaceError) as exc:
    ...
    def make_auth(self, handshake, username=None, password=None, database=None,
        ...
        """Make a GBase Authentication packet"""
    ...
    def make_command(self, command, argument=None):
        """Make a GBase packet containing a command"""
    ...
    def make_stmt_fetch(self, statement_id, rows=1):
        """Make a GBase packet with Fetch Statement command"""
    ...
    def make_change_user(self, handshake, username=None, password=None,
        ...
        """Make a GBase packet with the Change User command"""
    ...
    def parse_handshake(self, packet):
        """Parse a GBase Handshake-packet"""
        res = {}
        ...
        if capabilities & ClientFlag.PLUGIN_AUTH:
            if (b'\x00' not in packet
                ...
                # GBase server 5.5.8 has a bug where end byte is not send
            ...
            res['auth_plugin'] = 'gbase_native_password'
    ...
    def parse_ok(self, packet):
        """Parse a GBase OK-packet"""
        if not packet[4] == 0:
            raise GBaseError.InterfaceError("Failed parsing OK packet (invalid).")
    ...
    def parse_column_count(self, packet):
        """Parse a GBase packet with the number of columns in result set"""
        try:
            count = utils.read_lc_int(packet[4:])[1]
            return count
        except (struct.error, ValueError):
            raise GBaseError.InterfaceError("Failed parsing column count")

    def parse_column(self, packet, charset='utf-8'):
        """Parse a GBase column-packet"""
        ...
        try:
            ...
            raise GBaseError.InterfaceError("Failed parsing column information")
        ...
        return (
            ...
            flags,  # GBase specific
        )

    def parse_eof(self, packet):
        """Parse a GBase EOF-packet"""
        if packet[4] == 0:
        ...
        try:
            ...
            raise GBaseError.InterfaceError(err_msg)
    ...
    def read_binary_result(self, sock, columns, count=1, charset='utf-8'):
        """Read GBase binary protocol result
    ...
    def parse_binary_prepare_ok(self, packet):
        """Parse a GBase Binary Protocol OK packet"""
        if not packet[4] == 0:
            raise GBaseError.InterfaceError("Failed parsing Binary OK packet")
    ...
    def _prepare_binary_integer(self, value):
        """Prepare an integer for the GBase binary protocol"""
        field_type = None
        flags = 0
        if value < 0:
            if value >= -128:
    ...
    def _prepare_binary_timestamp(self, value):
        """Prepare a timestamp object for the GBase binary protocol

        This method prepares a timestamp of type datetime.datetime or
        datetime.date for sending over the GBase binary protocol.
    ...
    def _prepare_binary_time(self, value):
        """Prepare a time object for the GBase binary protocol

        This method prepares a time object of type datetime.timedelta or
        datetime.time for sending over the GBase binary protocol.
        A tuple is returned with the prepared value and field type
        as elements.
    ...
    def make_stmt_execute(self, statement_id, data=(), parameters=(),
        ...
        """Make a GBase packet with the Statement Execute command"""
        iteration_count = 1
        ...
        if parameters and data:
            ...
            for pos, _ in enumerate(parameters):
                ...
                if value is None:
                    ...
                    raise GBaseError.ProgrammingError(
                        "GBase binary protocol can not handle "
    ...
    def parse_auth_switch_request(self, packet):
        """Parse a GBase AuthSwitchRequest-packet"""
        if not packet[4] == 254:
            raise GBaseError.InterfaceError(
    ...
    def parse_auth_more_data(self, packet):
        """Parse a GBase AuthMoreData-packet"""
        if not packet[4] == 1:
            raise GBaseError.InterfaceError(

...
# Logging
LOGGER = logging.getLogger("gbase_c_api_info")
...
# Import gbase.connector.version
#version_py = os.path.join("lib", "gbase", "connector", "version.py")
#version_py = "D:\\work\\td\\python\\gbase-connector-python-8.0.20\\lib\\gbase\\connector\\version.py"
...
CEXT_OPTIONS = [
    ('with-gbase-capi=', None,
     "Location of GBase C API installation or path to gbase_config"),
    ...
    if os.isdir(lib_file):
        ...
        for root, _, files in os.walk(lib_file):
            for filename in files:
                ...
                if filename.startswith('libgbaseclient') and \
                   not os.path.islink(filepath) and \
                   '_r' not in filename and \
                   '.a' not in filename:
                    gbaseclient_libs.append(filepath)
            if gbaseclient_libs:
                break
        # give priority to .so files instead of .a
        gbaseclient_libs.sort()
        lib_file = gbaseclient_libs[-1]
...
def parse_command_line(line, debug = False):
    ...
    command lines "gbase_config" might output. But it should be close enbough
    for our usage.
    """
...
def gbase_c_api_info(gbase_config, debug=False):
    """Get GBase information using gbase_config tool.

    Returns a dict.
    """

    process = Popen([gbase_config], stdout=PIPE, stderr=PIPE)
    ...
    for line in stdout.splitlines():
        ...
        if re_obj:
            ...
            # We always add the raw output from the different "gbase_config"
...
class BuildExtDynamic(build_ext):
    ...
    arch = None
    _gbase_config_info = None

    def initialize_options(self):
        build_ext.initialize_options(self)
        self.extra_compile_args = None
        self.extra_link_args = None
        self.with_gbase_capi = None
        self.with_gbasexpb_cext = False
    ...
    def _get_posix_openssl_libs(self):
        openssl_libs = []
        try:
            openssl_libs_path = os.path.join(self.with_gbase_capi, "lib")
            openssl_libs.extend([
            ...
            log.error("Couldn't find OpenSSL libraries in libgbaseclient")
        return openssl_libs

    def _copy_vendor_libraries(self):
        is_wheel = getattr(self.distribution.get_command_obj("install"),
                           "is_wheel", False)
        if not self.with_gbase_capi or not is_wheel:
            return
        ...
        if os.name == "nt":
            gbase_capi = os.path.join(self.with_gbase_capi, "bin")
            if ARCH_64BIT:
                vendor_libs.append((gbase_capi, ["libssl-1_1-x64.dll",
                                                 "libcrypto-1_1-x64.dll"]))
            else:
                vendor_libs.append((gbase_capi, ["libssl-1_1.dll",
                                                 "libcrypto-1_1.dll"]))
            # Bundle libgbase.dll
            src = os.path.join(self.with_gbase_capi, "lib", "libgbase.dll")
            dst = os.getcwd()
            log.info("copying {0} -> {1}".format(src, dst))
            shutil.copy(src, dst)
            data_files.append("libgbase.dll")
        else:
            gbase_config = self.with_gbase_capi \
                if not os.path.isdir(self.with_gbase_capi) \
                else os.path.join(self.with_gbase_capi, "bin", "gbase_config")
            gbase_info = gbase_c_api_info(gbase_config)
            if gbase_info["version"] >= (8, 0, 6):
                gbase_capi = os.path.join(self.with_gbase_capi, "lib")
                vendor_libs.append((gbase_capi, self._get_posix_openssl_libs()))
                vendor_folder = "gbase-vendor"

        if vendor_folder:
            mkpath(os.path.join(os.getcwd(), vendor_folder))

        # Copy vendor libraries to 'gbase-vendor' folder
    ...
    def _finalize_connector_c(self, connc_loc):
        ...
        self._gbase_config_info = None
        min_version = BuildExtDynamic.min_connector_c_version

        err_invalid_loc = "GBase C API location is invalid; was %s"

        gbase_config = None
        err_version = "GBase C API {0}.{1}.{2} or later required".format(
        ...
        if os.path.isdir(connc_loc):
            # if directory, and no gbase_config is available, figure out the
            # lib/ and include/ folders from the the filesystem
            gbase_config = os.path.join(connc_loc, 'bin', 'gbase_config')
            if os.path.isfile(gbase_config) and \
                    os.access(gbase_config, os.X_OK):
                connc_loc = gbase_config
                ...
                if not os.path.exists(myversionh):
                    log.error("GBase C API installation invalid "
                              "(gbase_version.h not found)")
                    sys.exit(1)
                else:
                    with open(myversionh, 'rb') as fp:
                        for line in fp.readlines():
                            if b'#define LIBGBASE_VERSION' in line:
                ...
                # On Windows we check libgbase.dll
                if os.name == 'nt':
                    lib = os.path.join(self.with_gbase_capi, 'lib',
                                       'libgbase.dll')
                    connc_64bit = win_dll_is64bit(lib)
                # On OSX we check libgbaseclient.dylib
                elif 'macos' in platform:
                    lib = os.path.join(self.with_gbase_capi, 'lib',
                                       'libgbaseclient.dylib')
                    connc_64bit = unix_lib_is64bit(lib)
                # On other Unices we check libgbaseclient (follow symlinks)
                elif os.name == 'posix':
        ...
        # We were given the location of the gbase_config tool (not on Windows)
        if not os.name == 'nt' and os.path.isfile(connc_loc) \
                and os.access(connc_loc, os.X_OK):
            gbase_config = connc_loc
            # Check gbase_config
            gbase_info = gbase_c_api_info(gbase_config)
            log.debug("# gbase_info: {0}".format(gbase_info))

            if gbase_info['version'] < min_version:
                log.error(err_version)
                sys.exit(1)

            include_dirs = gbase_info['include_directories']
            libraries = gbase_info['libraries']
            library_dirs = gbase_info['link_directories']
            self._gbase_config_info = gbase_info
            self.arch = self._gbase_config_info['arch']
    ...
    def finalize_options(self):
        self.set_undefined_options(
            ...
            ('with_gbase_capi', 'with_gbase_capi'),
        ...
        if self.with_gbase_capi:
            self._finalize_connector_c(self.with_gbase_capi)

        if not self.with_protobuf_include_dir:
            self.with_protobuf_include_dir = \
                os.environ.get("GBASEXPB_PROTOBUF_INCLUDE_DIR")

        if not self.with_protobuf_lib_dir:
            self.with_protobuf_lib_dir = \
                os.environ.get("GBASEXPB_PROTOBUF_LIB_DIR")

        if not self.with_protoc:
            self.with_protoc = os.environ.get("GBASEXPB_PROTOC")

        self.with_gbasexpb_cext = any((self.with_protobuf_include_dir,
                                       self.with_protobuf_lib_dir,
                                       self.with_protoc))
    ...
    def run_protoc(self):
        ...
        base_path = os.path.join(os.getcwd(), "src", "gbasexpb", "gbasex")
    ...
    def fix_compiler(self):
        ...
        for ext in self.extensions:
            ...
            if ext.name == "_gbasexpb" and self.with_gbasexpb_cext:
            ...
            # Add extra link args
            if self.extra_link_args and ext.name == "_gbase_connector":
                ...
                if platform.system() == "Linux":
                    ...
                    extra_link_args.extend(
                        ["-L{0}".format(lib)
                         for lib in gbase_info["link_directories"]])
                    extra_link_args += ["-Wl,-rpath,$ORIGIN/gbase-vendor"]
    ...
    def run(self):
        ...
        if not self.with_gbase_capi and not self.with_gbasexpb_cext:
            return
        ...
        if self.with_gbase_capi:
            ...
            if os.name != "nt":
                # Get GBase info
                gbase_capi = self.with_gbase_capi
                gbase_config = os.path.join(gbase_capi, "bin", "gbase_config") \
                    if os.path.isdir(gbase_capi) else gbase_capi
                gbase_info = gbase_c_api_info(gbase_config)
                gbase_version = "{}.{}.{}".format(*gbase_info["version"][:3])
            ...
            with open(os.path.join("docs", "INFO_BIN"), "w") as info_bin:
                ...
                if gbase_version:
                    info_bin.write("gbase-version: {}\n".format(gbase_version))
        ...
        if os.name == 'nt':
            for ext in self.extensions:
                ...
                if self.extra_compile_args:
                    ext.extra_compile_args.extend(self.extra_compile_args.split())
                # Add extra link args
                if self.extra_link_args and ext.name != "_gbasexpb":
                    ext.extra_link_args.extend(self.extra_link_args.split())
            if self.with_gbasexpb_cext:
                self.run_protoc()
            build_ext.run(self)
        else:
            ...
            if self.with_gbase_capi:
                copy_openssl = gbase_info["version"] >= (8, 0, 6)
                if platform.system() == "Darwin" and copy_openssl:
                    libssl, libcrypto = self._get_posix_openssl_libs()
                    cmd_libssl = [
                        "install_name_tool", "-change", libssl,
                        "@loader_path/gbase-vendor/{0}".format(libssl),
                        build_ext.get_ext_fullpath(self, "_gbase_connector")
                    ]
                    ...
                    cmd_libcrypto = [
                        ...
                        "@loader_path/gbase-vendor/{0}".format(libcrypto),
                        build_ext.get_ext_fullpath(self, "_gbase_connector")
                    ]
        ...
        if self.with_gbase_capi and self.compiler:
...
class BuildExtStatic(BuildExtDynamic):
    ...
    def finalize_options(self):
        ...
        install_obj.with_gbase_capi = self.with_gbase_capi
        ...
        if not self.with_gbase_capi:
            options_pairs.append(('with_gbase_capi', 'with_gbase_capi'))
        if not self.with_protobuf_include_dir:
    ...
    def _finalize_connector_c(self, connc_loc):
        if not os.path.isdir(connc_loc):
            log.error("GBase C API should be a directory")
            sys.exit(1)

        log.info("Copying GBase libraries")
        copy_tree(os.path.join(connc_loc, 'lib'), self.connc_lib)
        log.info("Copying GBase header files")
        copy_tree(os.path.join(connc_loc, 'include'), self.connc_include)

        # Remove all but static libraries to force static linking
        if os.name == 'posix':
            log.info("Removing non-static GBase libraries from %s" % self.connc_lib)
            ...
            self.libraries.extend(['libgbase'])
    ...
    def fix_compiler(self):
        ...
        if os.name == 'posix':
            ...
            if self.with_gbase_capi:
                libraries.append("gbaseclient")

            # As we statically link and the "libgbaseclient.a" library
...
class Install(install):
    ...
    description = "install GBase Connector/Python"
    ...
    def finalize_options(self):
        ...
        build_ext_obj.with_gbase_capi = self.with_gbase_capi
        ...
        if not self.need_ext:
            remove_cext(self.distribution, "_gbase_connector")

        self.with_gbasexpb_cext = all((self.with_protobuf_include_dir,
                                       self.with_protobuf_lib_dir,
                                       self.with_protoc))

        if not self.with_gbasexpb_cext:
            remove_cext(self.distribution, "_gbasexpb")

        install.finalize_options(self)

    def run(self):
        if not self.need_ext:
            log.info("Not Installing GBase C Extension")
        else:
            log.info("Installing GBase C Extension")
        install.run(self)

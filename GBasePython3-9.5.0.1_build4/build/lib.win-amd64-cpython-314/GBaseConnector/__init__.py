...
"""
GBase Connector/Python - GBase driver written in Python
"""

try:
    import _gbase_connector  # pylint: disable=F0401
    from .connection_cext import CGBaseConnection
except ImportError:
    HAVE_CEXT = False
else:
    HAVE_CEXT = True
...
from .GBaseConstants import DEFAULT_CONFIGURATION
from .GBaseError import (  # pylint: disable=W0622
...
from .GBaseConstants import FieldFlag, FieldType, CharacterSet, \
...
def _get_pooled_connection(**kwargs):
    """Return a pooled GBase connection"""
    # If no pool name specified, generate one
    from .pooling import (
        GBaseConnectionPool, generate_pool_name,
    ...
    with CONNECTION_POOL_LOCK:
        if pool_name not in _CONNECTION_POOLS:
            _CONNECTION_POOLS[pool_name] = GBaseConnectionPool(**kwargs)
        elif isinstance(_CONNECTION_POOLS[pool_name], GBaseConnectionPool):
...
def _get_failover_connection(**kwargs):
    """Return a GBase connection and try to failover if needed

    An InterfaceError is raise when no GBase is available. ValueError is
    ...
    Returns GBaseConnection instance.
    """
    config = kwargs.copy()
    try:
...
def connect(*args, **kwargs):
    """Create or get a GBase connection object

    In its simpliest form, Connect() will open a connection to a
    GBase server and return a GBaseConnection object.

    When any connection pooling arguments are given, for example pool_name
    or pool_size, a pool is created or a previously one is used to return
    a PooledGBaseConnection.

    Returns GBaseConnection or PooledGBaseConnection.
    """
    # DNS SRV
    ...
    if dns_srv:
        if not HAVE_DNSPYTHON:
            raise InterfaceError('GBase host configuration requested DNS '
    ...
    # Pooled connections
    try:
        from .GBaseConstants import CNX_POOL_ARGS
    ...
    if 'use_pure' in kwargs:
        ...
        if not use_pure and not HAVE_CEXT:
            raise ImportError("GBase Connector/Python C Extension not "
                              "available")

    if HAVE_CEXT and not use_pure:
        return CGBaseConnection(*args, **kwargs)
...
__all__ = [
    'GBaseConnection', 'Connect', 'custom_error_exception',

"""Implementing communication with GBase servers.
"""
...
from .conversion import GBaseConverter
from .cursor import (
    CursorBase, GBaseCursor, GBaseCursorRaw,
    GBaseCursorBuffered, GBaseCursorBufferedRaw, GBaseCursorPrepared,
    GBaseCursorDict, GBaseCursorBufferedDict, GBaseCursorNamedTuple,
    GBaseCursorBufferedNamedTuple)
from .network import GBaseUnixSocket, GBaseTCPSocket
from .protocol import GBaseProtocol
from .utils import int4store, linux_distribution
from .abstracts import GBaseConnectionAbstract


class GBaseConnection(GBaseConnectionAbstract):
    """Connection to a GBase Server"""
    def __init__(self, *args, **kwargs):
        self._protocol = None
        self._socket = None
        self._handshake = None
        super(GBaseConnection, self).__init__(*args, **kwargs)

        self._converter_class = GBaseConverter
    ...
    def _add_default_conn_attrs(self):
        ...
        default_conn_attrs = {
            ...
            "_client_name": "gbase-connector-python",
    ...
    def _do_handshake(self):
        """Get the handshake from the GBase server"""
        packet = self._socket.recv()
        if packet[4] == 255:
            raise GBaseError.get_exception(packet)
        ...
        try:
            ...
            raise GBaseError.get_gbase_exception(msg=err.msg, errno=err.errno,
        ...
        if not handshake['capabilities'] & ClientFlag.SSL:
            ...
            if self._ssl.get('verify_cert'):
                raise GBaseError.InterfaceError("SSL is required but the server "
    ...
    def _do_auth(self, username=None, password=None, database=None,
        ...
        """Authenticate with the GBase server

        Authentication happens in two parts. We first send a response to the
        handshake. The GBase server will then send either an AuthSwitchRequest
        or an error packet.

        Raises NotSupportedError when we get the old, insecure password
        reply back. Raises any error coming from GBase.
        """
        self._ssl_active = False
    ...
    def _auth_switch_request(self, username=None, password=None):
        ...
        if packet[4] == 254 and len(packet) == 5:
            raise GBaseError.NotSupportedError(
                ...
                "Password Hashing in the latest GBase manual")
        elif packet[4] == 254:
            # AuthSwitchRequest
            (new_auth_plugin,
            ...
            packet = self._socket.recv()
        elif packet[4] == 253:#增加逻辑，当返回头尾253时用sha256加密再重新发送
    ...
    def _get_connection(self, prtcls=None):
        ...
        Returns subclass of GBaseBaseSocket.
        """
        conn = None
        if self.unix_socket and os.name != 'nt':
            conn = GBaseUnixSocket(unix_socket=self.unix_socket)
        else:
            hosts = self.server_host
            if isinstance(self.server_host, str):
                hosts = [self.server_host]

            for host in hosts:
                try:
                    conn = GBaseTCPSocket(host=host,
    ...
    def _open_connection(self):
        """Open the connection to the GBase server

        This method sets up and opens the connection to the GBase server.

        Raises on errors.
        """
        self._protocol = GBaseProtocol()
    ...
    def shutdown(self):
        """Shut down connection to GBase Server.
        """
        if not self._socket:
            return
        ...
        except (AttributeError, GBaseError.Error):
    ...
    def _send_cmd(self, command, argument=None, packet_number=0, packet=None,
        ...
        """Send a command to the GBase server
        ...
        Some commands might not result in the GBase server returning
        a response. If a command does not return anything, you should
        set expect_response to False. The _send_cmd method will then
        return None instead of a GBase packet.

        Returns a GBase packet or None.
        """
    ...
    def _handle_server_status(self, flags):
        """Handle the server flags found in GBase packets

        This method handles the server flags send by GBase OK and EOF
    ...
    def _handle_ok(self, packet):
        """Handle a GBase OK packet

        This method handles a GBase OK packet. When the packet is found to
        ...
        if packet[4] == 0:
            ...
            raise GBaseError.get_exception(packet)
        raise GBaseError.InterfaceError('Expected OK packet')

    def _handle_eof(self, packet):
        """Handle a GBase EOF packet

        This method handles a GBase EOF packet. When the packet is found to
        be an Error packet, an error will be raised. If the packet is neither
    ...
    def _handle_result(self, packet):
        """Handle a GBase Result

        This method handles a GBase result, for example, after sending the
        ...
        if not packet or len(packet) < 4:
            raise GBaseError.InterfaceError('Empty response')
        elif packet[4] == 0:
            return self._handle_ok(packet)
        elif packet[4] == 251:
            if PY2:
        ...
        if not column_count or not isinstance(column_count, int):
            raise GBaseError.InterfaceError('Illegal result set.')
    ...
    def get_rows(self, count=None, binary=False, columns=None, raw=None,
                 prep_stmt=None):
        """Get all rows returned by the GBase server

        This method gets all rows returned by the GBase server after sending,
        ...
        try:
            if binary:
                ...
                rows = self._protocol.read_text_result(self._socket,
                                                       ...
                                                       count=count)
        except GBaseError.Error as err:
    ...
    def cmd_query(self, query, raw=False, buffered=False, raw_as_string=False):
        ...
        This method send the query to the GBase server and returns the result.
    ...
    def cmd_query_iter(self, statements):
        """Send one or more statements to the GBase server
        ...
        GBase server and through the iterator you can get the results.
    ...
    def cmd_refresh(self, options):
        ...
        This method sends the Refresh command to the GBase server. The options
        argument should be a bitwise value using constants.RefreshOption.
        Usage example:
         RefreshOption = GBaseConnector.RefreshOption
    ...
    def cmd_quit(self):
        ...
        This method sends the QUIT command to the GBase server, closing the
    ...
    def cmd_shutdown(self, shutdown_type=None):
        ...
        This method sends the SHUTDOWN command to the GBase server and is only
    ...
    def cmd_statistics(self):
        """Send the statistics command to the GBase Server

        This method sends the STATISTICS command to the GBase server. The
    ...
    def cmd_process_kill(self, gbase_pid):
        ...
        """
        return self._handle_ok(
            self._send_cmd(ServerCmd.PROCESS_KILL, int4store(gbase_pid)))

    def cmd_debug(self):
        """Send the DEBUG command

        This method sends the DEBUG command to the GBase server, which
        requires the GBase user to have SUPER privilege. The output will go
        to the GBase server error log and the result of this method is a
    ...
    def cmd_ping(self):
        ...
        This method sends the PING command to the GBase server. It is used to
    ...
    def cmd_change_user(self, username='', password='', database='',
        ...
        if self._compress:
            raise GBaseError.NotSupportedError("Change user is not supported with "
                                           "compression.")
    ...
    def is_connected(self):
        ...
        This method checks whether the connection to GBase is available.
    ...
    def reset_session(self, user_variables=None, session_variables=None):
        ...
        This method resets the session state, if the GBase server is 5.7.3
        ...
        except GBaseError.NotSupportedError:
    ...
    def reconnect(self, attempts=1, delay=0):
        """Attempt to reconnect to the GBase server
        ...
        you expect the GBase server to be down for maintenance or when you
        ...
        while counter != attempts:
            ...
            try:
                ...
                if counter == attempts:
                    msg = "Can not reconnect to GBase after {0} "\
                          "attempt(s): {1}".format(attempts, str(err))
                    raise GBaseError.InterfaceError(msg)
            if delay > 0:
                time.sleep(delay)
    ...
    def ping(self, reconnect=False, attempts=1, delay=0):
        ...
        try:
            ...
            if reconnect:
                ...
                raise GBaseError.InterfaceError("Connection to GBase is"
                                            " not available.")
    ...
    def cursor(self, buffered=None, raw=None, prepared=None, cursor_class=None,
        ...
        By default, GBaseCursor is returned. Depending on the options
        ...
        GBaseConnector.cursor.CursorBase.
        ...
        types = {
            0: GBaseCursor,  # 0
            1: GBaseCursorBuffered,
            2: GBaseCursorRaw,
            3: GBaseCursorBufferedRaw,
            4: GBaseCursorDict,
            5: GBaseCursorBufferedDict,
            8: GBaseCursorNamedTuple,
            9: GBaseCursorBufferedNamedTuple,
            16: GBaseCursorPrepared
        }
        try:
    ...
    def _handle_binary_ok(self, packet):
        """Handle a GBase Binary Protocol OK packet

        This method handles a GBase Binary Protocol OK packet. When the
        ...
        if packet[4] == 0:
            ...
            raise GBaseError.get_exception(packet)
        raise GBaseError.InterfaceError('Expected Binary OK packet')
    ...
    def cmd_stmt_fetch(self, statement_id, rows=1):
        """Fetch a GBase statement Result Set

        This method will send the FETCH command to GBase together with the
    ...
    def cmd_stmt_prepare(self, statement):
        """Prepare a GBase statement

        This method will send the PREPARE command to GBase together with the
        given statement.
    ...
    def cmd_stmt_execute(self, statement_id, data=(), parameters=(), flags=0):
        """Execute a prepared GBase statement"""
    ...
    def cmd_stmt_close(self, statement_id):
        """Deallocate a prepared GBase statement

        This method deallocates the prepared statement using the
        statement_id. Note that the GBase server does not return
        anything.
        """
    ...
    def cmd_stmt_send_long_data(self, statement_id, param_id, data):
        ...
        Since GBase does not send anything back, no error is raised. When
        the GBase server is not reachable, an OperationalError is raised.
    ...
    def cmd_reset_connection(self):
        ...
        Works only for GBase server 5.7.3 or later.
        The result is a dictionary with OK packet information.

        Returns a dict()
        """
        if self._server_version < (5, 7, 3):
            raise GBaseError.NotSupportedError("GBase version 5.7.2 and "

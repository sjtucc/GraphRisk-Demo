...
from _gbase_connector import GBaseInterfaceError  # pylint: disable=F0401,E0611
...
class CGBaseCursor(GBaseCursorAbstract):

    """Default cursor for interacting with GBase using C Extension"""
    ...
    def __init__(self, connection):
        """Initialize"""
        GBaseCursorAbstract.__init__(self)
        ...
        if not isinstance(connection, GBaseConnectionAbstract):
    ...
    def _handle_result(self, result):
        ...
        if 'columns' in result:
            ...
            if self._cnx.raise_on_warnings is True and self._warnings:
                raise GBaseError.get_gbase_exception(*self._warnings[0][1:3])
    ...
    def _execute_iter(self):
        ...
        while True:
            try:
                if not self.nextset():
                    raise StopIteration
            except GBaseError.InterfaceError as exc:
    ...
    def execute(self, operation, params=(), multi_stmt=False):
        ...
        Deprecated: The multi_stmt argument is not needed and nextset() should
        ...
        if params:
            prepared = self._cnx.prepare_for_gbase(params)
    ...
    def callproc(self, procname, args=()):
        ...
        try:
            ...
            while self._cnx.result_set_available:
                ...
                if self._raw:
                    cur = CGBaseCursorBufferedRaw(self._cnx._get_self())
                else:
                    cur = CGBaseCursorBuffered(self._cnx._get_self())
    ...
    def nextset(self):
        ...
        if not self._cnx.result_set_available:
            ...
            raise GBaseError.InterfaceError(errno=CR_NO_RESULT_SET)
...
class CGBaseCursorBuffered(CGBaseCursor):

    """Cursor using C Extension buffering results"""

    def __init__(self, connection):
        """Initialize"""
        super(CGBaseCursorBuffered, self).__init__(connection)
    ...
    def reset(self, free=True):
        ...
        self._rows = None
        self._next_row = 0
        super(CGBaseCursorBuffered, self).reset(free=free)
...
class CGBaseCursorRaw(CGBaseCursor):
...
class CGBaseCursorBufferedRaw(CGBaseCursorBuffered):
...
class CGBaseCursorDict(CGBaseCursor):
    ...
    def fetchone(self):
        """Returns all rows of a query result set
        """
        row = super(CGBaseCursorDict, self).fetchone()
        if row:
    ...
    def fetchmany(self, size=1):
        ...
        res = super(CGBaseCursorDict, self).fetchmany(size=size)
        return [dict(zip(self.column_names, row)) for row in res]

    def fetchall(self):
        """Returns all rows of a query result set as list of dictionaries"""
        res = super(CGBaseCursorDict, self).fetchall()
        return [dict(zip(self.column_names, row)) for row in res]


class CGBaseCursorBufferedDict(CGBaseCursorBuffered):
    ...
    def _fetch_row(self):
        row = super(CGBaseCursorBufferedDict, self)._fetch_row()
        if row:
            return dict(zip(self.column_names, row))
        return None

    def fetchall(self):
        res = super(CGBaseCursorBufferedDict, self).fetchall()
        return [dict(zip(self.column_names, row)) for row in res]


class CGBaseCursorNamedTuple(CGBaseCursor):

    """Cursor using C Extension returning rows as named tuples"""

    def _handle_resultset(self):
        """Handle a result set"""
        super(CGBaseCursorNamedTuple, self)._handle_resultset()
    ...
    def fetchone(self):
        """Returns all rows of a query result set
        """
        row = super(CGBaseCursorNamedTuple, self).fetchone()
        if row:
    ...
    def fetchmany(self, size=1):
        ...
        res = super(CGBaseCursorNamedTuple, self).fetchmany(size=size)
        return [self.named_tuple(*res[0])]

    def fetchall(self):
        """Returns all rows of a query result set as list of named tuples"""
        res = super(CGBaseCursorNamedTuple, self).fetchall()
        return [self.named_tuple(*row) for row in res]


class CGBaseCursorBufferedNamedTuple(CGBaseCursorBuffered):

    """Cursor using C Extension buffering and returning rows as named tuples"""

    def _handle_resultset(self):
        super(CGBaseCursorBufferedNamedTuple, self)._handle_resultset()
        # pylint: disable=W0201
        self.named_tuple = namedtuple('Row', self.column_names)
        # pylint: enable=W0201

    def _fetch_row(self):
        row = super(CGBaseCursorBufferedNamedTuple, self)._fetch_row()
        if row:
            return self.named_tuple(*row)
        return None

    def fetchall(self):
        res = super(CGBaseCursorBufferedNamedTuple, self).fetchall()
        return [self.named_tuple(*row) for row in res]


class CGBaseCursorPrepared(CGBaseCursor):

    """Cursor using GBase Prepared Statements"""

    def __init__(self, connection):
        super(CGBaseCursorPrepared, self).__init__(connection)
        self._rows = None
        self._rowcount = 0
        self._next_row = 0
        self._binary = True
    ...
    def callproc(self, procname, args=None):
        ...
        Not supported with CGBaseCursorPrepared.
        """
    ...
    def close(self):
        ...
        if self._stmt:
            ...
            self._stmt = None
        super(CGBaseCursorPrepared, self).close()

    def reset(self, free=True):
        """Resets the prepared statement."""
        if self._stmt:
            self._cnx.cmd_stmt_reset(self._stmt)
        super(CGBaseCursorPrepared, self).reset(free=free)
    ...
    def execute(self, operation, params=(), multi_stmt=False):  # multi_stmt is unused
        ...
        if operation is not self._executed:
            ...
            except (GBaseError.Error, GBaseError.ProgrammingError):
                self._stmt = None
                raise
